package com.sultonbek1547.namozvaqtlari
...
import com.sultonbek1547.namozvaqtlari.databinding.ActivityMainBinding
import com.sultonbek1547.namozvaqtlari.model.Taqvim
import com.sultonbek1547.namozvaqtlari.utils.MyConstants.networkState
import com.sultonbek1547.namozvaqtlari.utils.MyConstants.regions
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.tvRegion.setOnClickListener {
            regionPopupMenu.setOnMenuItemClickListener { menuItem ->
                ...
                binding.tvRegion.text = regions[selectedRegionIndex]
                urlDay =
                    "https://islomapi.uz/api/present/day?region=${regions[selectedRegionIndex]}"
                urlWeek =
                    "https://islomapi.uz/api/present/week?region=${regions[selectedRegionIndex]}"
                urlMonth =
                    "https://islomapi.uz/api/monthly?region=${regions[selectedRegionIndex]}&month="
                /**  getting new data*/
    ...
    private fun getAllThreeDataAtOnce() {
        ...
        requestQueue.add(object : JsonObjectRequest(Request.Method.GET, urlDay, null,
            { response ->
                ...
                loadData()
                currentRegionName = binding.tvRegion.text.toString()
    ...
    private fun findNextTime(givenTime: String) {
        ...
        nextPrayerTime = sortedTimeList[0].format(DateTimeFormatter.ofPattern("HH:mm"))
        binding.tvNameCurrent.text = nameList[1]
        isXufton = true
        /** starting timer */
        startTimer()
    ...
    private fun startTimer() {
        ...
        val targetDateTime = LocalDateTime.parse(
            "${currentItem.date} $nextPrayerTime:00",
            DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
        )
        val currentTime = LocalDateTime.now()

        val duration =
            if (targetDateTime.isAfter(currentTime)) Duration.between(currentTime, targetDateTime)
            else Duration.between(currentTime, targetDateTime.plusDays(1))
